...
**Main Topic:** Amazon EC2 (Elastic Compute Cloud
 

**Key Takeaways:** 
1. EC2 is essentially a virtual server in the cloud that I can fully control - from the operating system down to the applications running on it.  
2. Instance types matter because they determine how much CPU, memory, and network performance I get, and choosing the wrong one can cost money or performance
3. Security Groups and SSH key pairs are the gatekeepers - without them configured correctly, nothing connects, and nothing works. 

 

**New Terms:** 
- **EC2 Instance:** A virtual machine in AWS that I can configure, start, stop, and terminate as needed. 
- **AMI (Amazon Machine Image):** A preconfigured template that defines the operating system and initial software for an EC2 
...
**Result:**  
I successfully launched an EC2 instance, connected to it via SSH, installed and configured an Apache web server, and verified it was accessible through a public IPv4 address in the browser. I also practiced starting, stopping, enabling, and disabling the service to better understand instances and service lifecycle management. 
...
![Starting the Apache Web Server](./screenshots/apache-web-server-error.png) 

 

 

Checking the status of the Apache Server 
![Status Check](./screenshots/apache-web-server-status.png) 
...
![Enabling Server](./screenshots/apache-web-server-enable.png) 

 

Visited the Apache Web Server in the browser 
![Browser Check](./screenshots/apache-web-server-active.png) 

 

Disabling the Apache Web Server 
![Disabling Server](./screenshots/apache-web-server-disable-and-status-check.png) 

 

Stop the Apache Web Server 
![Stop the Apache Web Server](./screenshots/apache-web-server-stop.png) 
...
**How I'll use it:** 
I'll approach every future EC2 deployment with a checklist mindset: AMI, instance type, security group rules, and SSH access before assuming anything is broken 
...
**Problem:**  
After launching the EC2 instance, I couldn't connect to it via SSH from my local machine even though inbound SSH traffic was allowed. 

 

**Solution:**  
I discovered the issue was local, not AWS-related. The SSH key pair’s file permission needed to be changed to read-only for the file owner. 

 

**Lesson:**  
When troubleshooting EC2 access, always check both sides - AWS configuration and local system permissions 
...
- Apache Web Server - Status: Deactivate 
...
- [ ] Overview of S3 storage class 
- [ ] S3 bucket naming rules 

 

**Questions to Answer:** 
- When should I use S3 instead of EC2 storage? 
- How does S3 handle durability and availability 
...
**Daily Win:**  
Successfully launched an EC2 instance, fixed SSH access issues, and deployed a working Apache web server from scratch 

 

**Tomorrow's Promise:**  
I will treat storage as intentionally as compute and learn S3 with the same hands-on mindset 
